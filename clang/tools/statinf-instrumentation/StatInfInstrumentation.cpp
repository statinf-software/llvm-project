...
int main(int argc, const char **argv) {
    ...
    auto ExpectedParser =
        CommonOptionsParser::create(argc, argv, StatInfInstrCategory);
    if (!ExpectedParser) {
    ...
    // Because I need a file to call the buildAst.. function
    pair<string, string> last_file = files.back();
    files.pop_back();

    // Build ASTs
    std::unique_ptr<clang::ASTUnit> ast = buildASTFromCodeWithArgs(
      last_file.second, args, last_file.first,
      "statinf-instrumentation", PCHContainerOps,
      clang::tooling::getClangStripDependencyFileAdjuster(), 
      files
    );
    if(ast == nullptr) {
      llvm::errs() << "No AST have been built\n";
      return -1;
    }
    ...
    StatInfPrinterLauncher launch(new_code_stream, &cg);
